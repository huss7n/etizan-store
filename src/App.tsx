...
function AdminDashboard({ user, onLogout, onBack }: { user: any; onLogout: () => void; onBack: () => void }) {
  ...
  const checkAdminPermissions = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) setAdminPermissions(data.permissions);
    } catch (e) {
      console.error(e);
    }
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="hidden md:flex items-center gap-1 p-1 bg-white/[0.03] border border-white/10 rounded-xl">
                {[
                  ...
                  { id: 'products', label: 'المنتجات', icon: Package, count: products.length, hide: !adminPermissions?.canManageProducts },
                  { id: 'settings', label: 'الإعدادات', icon: Settings, count: 0, hide: !adminPermissions?.canManageSettings },
                  { id: 'admins', label: 'المشرفين', icon: LogOut, count: 0, hide: !adminPermissions?.canManageAdmins },
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="md:hidden flex items-center gap-1 p-1 bg-white/[0.03] border border-white/10 rounded-xl mb-6">
          {[
            ...
            { id: 'products', label: 'المنتجات', icon: Package, hide: !adminPermissions?.canManageProducts },
            { id: 'settings', label: 'الإعدادات', icon: Settings, hide: !adminPermissions?.canManageSettings },
            { id: 'admins', label: 'المشرفين', icon: LogOut, hide: !adminPermissions?.canManageAdmins },
